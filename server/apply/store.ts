...
function asItem(row: Record<string, unknown>, runId: string): AutoApplyQueueItem {
  return {
    ...
    jobDescriptionSnapshot: typeof row.job_description_snapshot === 'string' ? row.job_description_snapshot : null,
    location: typeof row.location === 'string' ? row.location : null,
    confirmationNumber: typeof row.confirmation_number === 'string' ? row.confirmation_number : null,
      ...
      Promise.resolve(
        supabase.from('auto_apply_queue').upsert(
          items.map((item) => ({
            ...
            job_description_snapshot: item.jobDescriptionSnapshot,
