...
function recommend(
  ...
  const majorityRequiredMissing =
    requiredCount > 0 && requiredMissing >= 2 && requiredMissing / requiredCount >= 0.5
  const experienceGap = experience === 'gap'
  if (score < 48 || majorityRequiredMissing || (experienceGap && score < 70)) {
    return 'SKIP'
  }
  if (
    score >= 78 &&
