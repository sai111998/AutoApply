...
export function syntheticSliceHtml(
  kind: 'job' | 'step1' | 'step2' | 'review' | 'confirm',
  base = '/test-employer/job/1',
): string {
  if (kind === 'job') {
    return `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8" /><title>Senior Java Full Stack Developer</title></head>
  <body>
  ...
  if (kind === 'step1') {
    return `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8" /><title>Application Step 1</title></head>
  <body>
    <h1>Application Step 1</h1>
    <form action="${base}/resume" method="get">
      <label for="first-name">First Name</label>
  ...
  if (kind === 'step2') {
    return `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8" /><title>Resume Upload</title></head>
  <body>
    <h1>Resume Upload</h1>
    <form action="${base}/review" method="post" enctype="multipart/form-data">
...
<html lang="en">
  ...
  <body>
    ...
    <p>Resume</p>
    <p>Job title</p>
    <p>Senior Java Full Stack Developer</p>
...
export function startSyntheticEmployer(port = 0): Promise<{
  ...
  return new Promise((resolve) => {
    const server = createServer((req, res) => {
      ...
      if (url.startsWith('/test-employer/job/1')) {
        const sliceBase = '/test-employer/job/1'
        if (url.endsWith('/submit')) {
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(syntheticSliceHtml('confirm', sliceBase))
          return
        }
        if (url.endsWith('/review')) {
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(syntheticSliceHtml('review', sliceBase))
          return
        }
        if (url.endsWith('/resume')) {
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(syntheticSliceHtml('step2', sliceBase))
          return
        }
        if (url.endsWith('/apply')) {
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(syntheticSliceHtml('step1', sliceBase))
          return
        }
        res.writeHead(200, { 'Content-Type': 'text/html' })
        res.end(syntheticSliceHtml('job', sliceBase))
        return
      }
